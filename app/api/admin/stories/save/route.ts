...
type Payload = Partial<{
  ...
  how: string;
  why: string;
  lifeLesson: string;          // camelCase in incoming payload
  city: string;
  state: string;
  country: string;
  hero_image: string;
...
const ALLOWED: (keyof Payload)[] = [
  "slug","title","dek","what","how","why","lifeLesson",
...
function coercePayload(raw: any): { id: string; update: Record<string, any> } {
  ...
  for (const k of ALLOWED) {
    ...
    if (k === "lifeLesson") {
      // camel → snake for DB
      update.life_lesson = typeof raw.lifeLesson === "string" ? raw.lifeLesson.trim() : raw.lifeLesson;
      continue;
    }
...
export async function POST(req: Request) {
  ...
  if (update.slug) {
    ...
    if (dup && dup.length > 0) return NextResponse.json({ error: "Slug already exists" }, { status: 409 });
  }
