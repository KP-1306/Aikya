...
function toEmbedText(row: any): string {
  // Prefer camelCase if present in application objects; DB read uses snake_case
  const parts = [
    row.title ?? "",
    row.dek ?? "",
    row.what ?? "",
    row.how ?? "",
    row.why ?? "",
    (row.lifeLesson ?? row.life_lesson ?? ""),
  ]
...
export async function POST(req: Request) {
  ...
  if (!rows?.length) return NextResponse.json({ ok: true, updated: 0, note: "No candidates" });

  const texts = rows.map(toEmbedText).map((t) => t || "");
  const payload = texts.map((t, i) => ({ i, t })).filter((x) => x.t.length > 0);
  if (!payload.length) return NextResponse.json({ ok: true, updated: 0, note: "No non-empty texts" });

  const vectors = await embed(payload.map((p) => p.t));
  if (!vectors.length) return NextResponse.json({ ok: false, error: "Embedding call failed" }, { status: 500 });
